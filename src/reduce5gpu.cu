...
int main(void)
{
    ...
    // printf("\nusing cooperative group, add global compute:\n");
    // timing(d_x);

    printf("\nusing cooperative group, add global compute, static global mem:\n");
    timing(d_x);
...
void __global__ reduce_cp(const real *d_x, real *d_y, const int N){
    ...
    // 将每个线程计算的累加结果 y 存储到 s_y 数组中的对应位置，索引为 tid
    s_y[tid] = y;
    __syncthreads();
...
real reduce_static(const real *d_x)
{
    // const int ymem = sizeof(real) * GRID_SIZE;
    const int smem = sizeof(real) * BLOCK_SIZE;
